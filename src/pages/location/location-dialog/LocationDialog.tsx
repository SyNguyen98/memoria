...
import {useCreateLocationMutation, useUpdateLocationMutation} from "@queries/useLocationQueries.ts";
import {
    Button,
    Dialog,
    DialogActions,
    ...
    const handleSave = () => {
        const onSuccess = () => {
            queryClient.invalidateQueries({queryKey: ['getPagingLocationsByParams', collectionId]}).then(() => {
                handleClose();
                hideAppLoader();
                showSnackbar("success", t('location.save_success'));
            });
        }
        const onError = () => {
            hideAppLoader();
            showSnackbar("error", t('location.save_error'));
        }
        ...
        if (props.location) {
            ...
            updateMutation.mutate(location, {onSuccess, onError});
        } else {
            createMutation.mutate(location, {onSuccess, onError});
        }
    }
