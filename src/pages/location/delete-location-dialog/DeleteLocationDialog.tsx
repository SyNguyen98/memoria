...
import {useDeleteLocationMutation} from "@queries/useLocationQueries.ts";
    ...
    const handleDeleteCollection = () => {
        deleteMutation.mutate(props.location!.id!, {
            onSuccess: () => {
                ...
                props.onClose();
                showSnackbar("success", "Đã xóa địa điểm");
                queryClient.invalidateQueries({queryKey: ['getPagingLocationsByParams', collectionId]}).catch(() => {
                    showSnackbar("error", "Không thể tải địa điểm");
                });
            },
            onError: () => {
                showSnackbar("error", "Không thể xóa địa điểm");
            }
        });
    }
