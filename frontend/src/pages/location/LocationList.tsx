...
import {usePagingLocationQuery} from "@queries/useLocationQueries.ts";
import {useCollectionByIdQuery} from "@queries/useCollectionQueries.ts";
import {
    Button,
    Card,
    CardActions,
...
function LocationList() {
    ...
    const numOfLocations = useMemo(() => {
        if (locationQuery.data) {
            return Number(locationQuery.data.header.get("x-total-count"))
        }
        return 0;
    }, [locationQuery.data]);
    ...
    const refreshLocations = (page: number, size: number) => {
        queryClient.invalidateQueries({queryKey: ['getPagingLocationsByParams', collectionId, page, size]}).catch(() => {
            showSnackbar("error", t("location.cannot_load"));
        });
    }
    ...
    const handleNavigateToItem = (location: Location) => {
        navigate(`${location.id}/${PathName.ITEM}`);
    }
