...
// Utils & Services
import {useCollectionQuery} from "@queries/useCollectionQueries.ts";
...
function CollectionList() {
    ...
    const numOfCollections = useMemo(() => {
        if (collectionQuery.data) {
            return Number(collectionQuery.data.header.get("x-total-count"))
        }
        return 0;
    ...
    useEffect(() => {
        ...
        if (collectionQuery.isError) {
            showSnackbar("error", t("collection.cannot_load"));
        }
    }, [collectionQuery.isError, showSnackbar, t]);
    ...
    const handleNavigateToLocation = (collection: Collection) => {
        navigate(`${collection.id}/${PathName.LOCATION}`);
    }
